...
public class TranslateBehavior extends FloatingActionButton.Behavior {
    //添加一个加速器实现弹射效果
    private FastOutLinearInInterpolator fastInterpolator = new FastOutLinearInInterpolator();
    ...
    public boolean onStartNestedScroll(CoordinatorLayout coordinatorLayout, FloatingActionButton child, View directTargetChild, View target, int nestedScrollAxes) {
        //开始滑动监听--当观察recyclerView开始发生滑动的回调
    ...
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, FloatingActionButton child, View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        if (dxConsumed > 0 && !isAnimatingOut && child.getVisibility() == View.VISIBLE) {
            //fab滑出去
            animateOut(child);
    ...
    private void animateIn(FloatingActionButton child) {
        ...
        //属性动画
        ViewCompat.animate(child).translationX(0).setInterpolator(fastInterpolator).setListener(null).start();
    }

    private void animateOut(FloatingActionButton child) {
        //属性动画
        //设置监听判断状态
        ViewCompat.animate(child).translationX(child.getHeight()).setInterpolator(fastInterpolator).setListener(new ViewPropertyAnimatorListenerAdapter(){
            @Override
